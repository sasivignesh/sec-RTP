...
static void test_cache_getSelfZID(void) {
	...
	bzrtpCache_t *aliceDB=NULL;
	uint8_t selfZIDalice[12];
	uint8_t selfZIDalice2[12];
	...
	int zuidalicebob=0, zuidCheck=0;
...
//	int i;
	...
	aliceDB = bzrtp_cacheOpen(aliceCacheFile);
	BC_ASSERT_PTR_NOT_NULL(aliceDB);
	if (aliceDB == NULL) {
		return;
	}
	/* check what happend if we try to run cacheless */
	BC_ASSERT_EQUAL(bzrtp_setZIDCache_new(aliceContext, NULL, "alice@sip.linphone.org", "bob@sip.linphone.org"),BZRTP_ZIDCACHE_RUNTIME_CACHELESS, int, "%x");
	/* add real cache data into context using the dedicated function */
	BC_ASSERT_EQUAL(bzrtp_setZIDCache_new(aliceContext, aliceDB, "alice@sip.linphone.org", "bob@sip.linphone.org"), BZRTP_CACHE_SETUP,int,"%x");
	...
	BC_ASSERT_EQUAL(bzrtp_getSelfZID(aliceContext->zidCache_new, aliceContext->selfURI, aliceContext->selfZID, aliceContext->RNGContext), 0, int, "%x");
	...
	/* get it again, in another buffer and compare */
	BC_ASSERT_EQUAL(bzrtp_getSelfZID(aliceDB, "alice@sip.linphone.org", selfZIDalice2, aliceContext->RNGContext), 0, int, "%x");
	BC_ASSERT_EQUAL(memcmp(selfZIDalice2, aliceContext->selfZID, 12), 0, int, "%d");
	...
	BC_ASSERT_EQUAL(bzrtp_cache_getZuid_new(aliceDB, "alice@sip.linphone.org", "bob@sip.linphone.org", patternZIDbob, BZRTP_ZIDCACHE_INSERT_ZUID, &zuidalicebob), 0, int, "%x");
	/* ask for it again and check they are the same */
	BC_ASSERT_EQUAL(bzrtp_cache_getZuid_new(aliceDB, "alice@sip.linphone.org", "bob@sip.linphone.org", patternZIDbob, BZRTP_ZIDCACHE_DONT_INSERT_ZUID, &zuidCheck), 0, int, "%x");
	...
	aliceDB = bzrtp_cacheOpen(patternFilename);
	BC_ASSERT_PTR_NOT_NULL(aliceDB);
	if (aliceDB == NULL) {
	...
	BC_ASSERT_EQUAL(bzrtp_cache_getZuid_new(aliceDB, "alice@sip.linphone.org", "bob@sip.linphone.org", patternZIDbob, BZRTP_ZIDCACHE_DONT_INSERT_ZUID, &zuidalicebob), 0, int, "%x");
...
static void test_cache_zrtpSecrets(void) {
	...
	/* get Secrets */
	int ret = bzrtp_getPeerAssociatedSecrets_new(aliceContext, peerZIDbob);
	BC_ASSERT_EQUAL(ret, 0, int, "%x");
	if (ret) return;
	...
	BC_ASSERT_EQUAL(bzrtp_setZIDCache_new(aliceContext, (void *)aliceDB, "alice@sip.linphone.org", "eve@sip.linphone.org"), BZRTP_CACHE_SETUP, int, "%x");
	ret = bzrtp_getPeerAssociatedSecrets_new(aliceContext, peerZIDbob);
	BC_ASSERT_EQUAL(ret, 0, int, "%x");
	if (ret) return;
